...
async function integrateBuildSystem(request: BuildIntegrationRequest): Promise<BuildIntegrationResult> {
  // Resolve paths
  ...
  const buildSystem = await determineBuildSystem(projectPath, request.buildSystem);

  // Generate build targets
  const buildTargets = await generateBuildTargets(projectPath, installPath, buildSystem, romOutputPath);

  // Generate build scripts
  const scriptsGenerated = request.generateScripts 
    ? await generateBuildScripts(projectPath, installPath, buildTargets, buildSystem)
    : [];
...
async function findPVSnesLibInstallation(): Promise<string> {
  ...
  throw new Error('PVSnesLib installation not found. Please specify installPath.');
}
...
async function generateBuildTargets(
  ...
  const targets: BuildTarget[] = [
    ...
    },
    {
      ...
      command: 'echo "Checking for SNES emulators..." && (which snes9x || which zsnes || echo "Consider installing: sudo apt-get install snes9x")',
      dependencies: [],
      outputs: [],
    },
  ];
...
async function generateMakefile(
  ...
  for (const target of buildTargets) {
    ...
    if (target.dependencies.length > 0) {
      makefileContent[makefileContent.length - 1] += ` ${target.dependencies.join(' ')}`;
    }
...
async function generateBuildScript(
  ...
  if (buildTarget) {
    ...
    buildScript.push('    echo "✅ Build successful! ROM created: $PROJECT.sfc"');
  ...
  try {
    ...
    const tasksJson = {
      ...
      tasks: buildTargets.map(target => ({
        ...
        group: target.name === 'build' ? { kind: 'build', isDefault: true } : undefined,
        options: {
          cwd: projectPath,
        },
    ...
    await fs.writeFile(join(vscodeDir, 'tasks.json'), JSON.stringify(tasksJson, null, 2));
    ...
    await fs.writeFile(join(vscodeDir, 'launch.json'), JSON.stringify(launchJson, null, 2));
    ...
    await fs.writeFile(join(vscodeDir, 'c_cpp_properties.json'), JSON.stringify(cppPropertiesJson, null, 2));
...
function formatBuildIntegrationResult(result: BuildIntegrationResult): string {
  ...
  lines.push(`   ${result.vscodeConfigured ? '✅' : '⚠️'} VS Code tasks and IntelliSense`);
